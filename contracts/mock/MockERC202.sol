...
contract MockERC202 is OwnableUpgradeable {
    ...
    function init(bytes calldata _implementationData) public initializer {
        // require(initialized == false, "Contract already initialized");

        // tokenTotalSupply = 100000000000000000000; // 100 tokens
        uint256 totalSupply = 100000000000000000000;
        address[] memory multipleOwners;

        (tokenName, tokenSymbol, tokenDecimals, multipleOwners) = abi.decode(
            _implementationData,
            (string, string, uint8, address[])
        );
        __Ownable_init_unchained();
        mint(multipleOwners[0], totalSupply.div(2));
        mint(multipleOwners[1], totalSupply.div(2));
    }
